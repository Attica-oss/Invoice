...
class DayNameType(PolarsEnum):
    ...
    @classmethod
    def special_day(cls) -> list[str]:
        """Returns a list of special days (PH and Sunday)."""
        return [cls.PH.value, cls.SUN.value]

    def is_special_day(self) -> bool:
        """Check if the day is a special day (PH or Sunday)."""
        return self.value in self.special_day()
...
def __get_public_holidays(year: int) -> list[date]:
    ...
    # Easter calculation (same logic you had)
    a = year % 19
    b = year // 100
    c = year % 100
...
def public_holiday(year: int = CURRENT_YEAR) -> pl.Series:
    ...
    all_days = (
        __get_public_holidays(prev_year)
        + __get_public_holidays(year)
        + __get_public_holidays(next_year)
    )
...
class Days:
    ...
    def add_day_name(self) -> pl.Expr:
        ...
        return (
            (
                pl.when(self._expr.is_in(public_holiday().implode()))
                .then(pl.lit("PH"))
