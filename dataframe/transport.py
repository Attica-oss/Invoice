...
Frames are built lazily by ``@lru_cache`` functions and exposed under their
historical names via :pep:`562` ``__getattr__`` -- importing this module does
no I/O.
"""
...
def _prices() -> dict[str, Any]:
    """Transport prices from one cached read of the price table."""
    return {
        # Need to make these LazyFrames + join_asof in case the price changes
        "shifting": get_price(["Shifting"]).get_column("unit_price")[0],
        "shore_crane": get_price(["Shore Crane"]).get_column("unit_price")[0],
        "transfer": get_price(["Haulage FEU", "Haulage TEU"]),
    }


@lru_cache(maxsize=1)
def _shore_crane() -> pl.LazyFrame:
    SHORE_CRANE_PRICE = _prices()["shore_crane"]
    return (
        ...
        .with_columns(
            (pl.col("hours") - pl.col("overtime_hours")).alias("normal_hours")
        )
        .with_columns(
            pl.when(
...
@lru_cache(maxsize=1)
def _transfer() -> pl.LazyFrame:
    SHIFTING_PRICE = _prices()["shifting"]
    TRANSFER_PRICE = _prices()["transfer"]
    return (
        ...
        .with_columns(
            shifting_price=pl.when(
                ...
                | (
                    (pl.col("remarks").eq("CCCS")).and_(
                        pl.col("driver").eq(pl.lit("NA"))
                    )
                )
            )
            .then(0)
...
@lru_cache(maxsize=1)
def _scow_transfer() -> pl.LazyFrame:
    return (
        load_gsheet_data(sheet_id=TRANSPORT_SHEET_ID, sheet_name=SCOW_TRANSFER_SHEET)
        .filter(pl.col("date").dt.year().eq(CURRENT_YEAR))
        .select(
            pl.col("date"),
            pl.col("container_number").cast(
                dtype=pl.Enum(["STDU6536343", "STDU6536338"])
            ),
            pl.col("customer"),
...
_EXPORTS = {
    "shore_crane": _shore_crane,
    "transfer": _transfer,
    "scow_transfer": _scow_transfer,
    "forklift": _forklift,
}
