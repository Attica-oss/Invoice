...
EXTRAMEN: pl.DataFrame = get_price(["Extra Men"]).with_columns(date=pl.col("date"))
...
ADDITIONAL_OVERTIME: pl.DataFrame = get_price(["Additional Overtime"]).with_columns(
    date=pl.col("date")
)
...
additional: pl.LazyFrame = (
    ...
    .sort(by="date")
    .join_asof(ADDITIONAL_OVERTIME.lazy(), by="service", on="date", strategy="backward")
    .with_columns(
