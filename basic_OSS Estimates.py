...
def _(full_oss_dataf, shore_crane):
    crane_dataf = mo.sql(
        ...
        FROM
            oss o
            LEFT JOIN crane c ON c.date BETWEEN o.starting_date AND o.ending_date
            AND o.vessel = c.vessel)

        SELECT id,vessel,COALESCE(SUM(total_price),0) AS total FROM joined GROUP BY id,vessel
        """
    )
    return (crane_dataf,)
        ...
        WITH
            ...
            net AS (
                SELECT
                    ...
                    vessel,
                    service,
                CASE WHEN overtime = 'normal hours' THEN 25.5 WHEN overtime = 'overtime 150%' THEN 38.25 ELSE 51
            END AS price_,
           total_tonnage AS tonnage,
            (price_ * tonnage) AS total
                FROM
                    netList
            WHERE YEAR(date) = 2025 AND service LIKE '%Basic%'
            ), joined AS (
...
@app.cell
def _(crane_dataf, full_oss_dataf, haulage_dataf, net_dataf, stuffing_dataf):
    _df = mo.sql(
        f"""
        WITH
            oss AS (
        ...
        FROM
            ...
            LEFT JOIN crane c ON c.id = o.id
