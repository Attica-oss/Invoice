...
def _():
    mo.md(r"""
    ### CCCS Container Stuffing (OSS::lucide:a-arrow-down::)
    """)
    return
...
def _():
    mo.md(r"""
    ### CCCS Stuffing others ::lucide:cable::
    """)
    return
...
def _():
    mo.md(r"""
    ### Shore Crane at CCCS for NON OSS ⛏️
    """)
    return
...
def _():
    mo.md(r"""
    ### Shore Crane for Full and Basic OSS
    """)
    return
...
def _():
    shore_crane_oss = mo.sql(
        ...
        WHERE Invoiced_to = 'MAERSKLINE' AND operation_type = 'OSS Stuffing'
        GROUP BY ALL
        ORDER BY month_number
        """
    )
    return (shore_crane_oss,)


@app.cell(hide_code=True)
def _():
    mo.md(r"""
    ### Electricity for CCCS Stuffed OSS
    """)
    return
    ...
    electricity = mo.sql(
        ...
        source AS (
            ...
            WHERE date_out IS NOT NULL
                AND operation_type = 'Stuffing and Plugin'
...
def _():
    mo.md(r"""
    ### Electricity for Basic and Full OSS
    """)
    return
...
def _():
    electricity_oss = mo.sql(
        ...
        source AS (
            ...
            FROM coa
            WHERE date_out IS NOT NULL
                AND (operation_type = 'Basic OSS' OR operation_type = 'Full OSS')
                AND YEAR(date_out) = 2026
        ),
        split AS (
            SELECT
    ...
    return (electricity_oss,)


@app.cell
def _(electricity_oss):
    transfer_oss = mo.sql(
        f"""
        WITH
            last_record AS (
...
def _(electricity):
    electricity_df = mo.sql(
        ...
        SELECT
            MONTH(invoice_month) AS month_number,
            MONTHNAME(invoice_month) AS month_name,
            REPLACE(REPLACE(vessel_client,'S.A.',''),'S.A','') AS vessel,
            customer,
            SUM(days_on_plug) AS total_days,
            CAST(SUM(total_price) AS DECIMAL) AS total_price
        GROUP BY ALL
...
@app.cell(hide_code=True)
def _(electricity_oss_2, shore_crane_oss, stuffing_oss, transfer_oss):
    _df = mo.sql(
        f"""
        WITH
            stuff AS (
            ...
            final AS (
                ...
                FROM
                    stuff s
                    LEFT JOIN shore x ON x.month_number = s.month_number
                    AND x.customer = s.vessel
                    LEFT JOIN elec e ON s.month_number = e.month_number
                    AND e.customer = s.vessel
                    LEFT JOIN haul h ON s.month_number = h.month_number
                    AND h.customer = s.vessel
            )
        FROM
            final
        SELECT
