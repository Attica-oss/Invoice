...
"""billing.py — shared billing expressions.

One implementation of the overtime rate multiplier, replacing the four
copy-pasted (and mutually inconsistent) versions in the old netlist
module. Import this from every pipeline that prices by overtime band —
including salt_operations — so the rates can never diverge again.

Fail-loud policy
----------------
An overtime label that isn't one of the three known values yields a
*null* multiplier, which nulls the price. The old code either billed
unknown labels at the normal rate (netList), at the overtime rate
(cccs "ERR" rows), or at zero (iot frames) — three different silent
wrong answers. Use :func:`unknown_overtime_rows` in a validation step
