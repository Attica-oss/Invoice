...
with app.setup:
    ...
    IOT_CARGO_PRICE = get_price(
        ["Loading to Cargo - Brine", "Loading to Cargo - Dry"]
    )
...
def _():
    ...
    OSS_STUFFING_PRICE: pl.DataFrame = get_price(
        oss_service_list
    ...
    return UNLOADING_PRICE, by_catch_companies
...
def _():
    stuffing_type = (
        ...
        .filter(
            ...
            & (
                pl.col("operation_type").str.contains_any(
                    ["Full", "Basic", "Stuffing"]
                )
            )
...
def _(cccs_record):
    cccs_adjusted_records = (
        (
            load_gsheet_data(
                sheet_id=OPS_SHEET_ID, sheet_name=RAW_DATA_SHEET_NAME
            )
            .filter(
            ...
            .with_columns(
                ...
                .otherwise(
                    pl.lit("ERR")
                )  # To modify this for the "Invalid invoice"
            )
        )
        .join(
            cccs_record,
        ...
        .with_columns(
            normal_tonnage=pl.col("total_tonnage_right")
            - pl.col("overtime_tonnage")
        )
        .with_columns(
        ...
        .with_columns(
            adjusted_tonnage=pl.col("total_tonnage") * pl.col("perc_diff")
        )
        .group_by(
            [
                "day_name",
    ...
    return (cccs_adjusted_records,)


@app.cell
def _(
    UNLOADING_PRICE: pl.DataFrame,
    by_catch_companies,
    cccs_adjusted_records,
    stuffing_type,
):
    netList = (
        pl.concat(
            [
                load_gsheet_data(
                    sheet_id=OPS_SHEET_ID, sheet_name=NET_LIST_SHEET_NAME
                )
                ...
                | (
                    ...
                    .and_(
                        pl.col("date").is_between(
                            date(2025, 11, 3), date(2025, 11, 15)
                        )
        ...
        .with_columns(
            service=pl.col("service") + " - " + pl.col("storage_type")
        )
        .sort(by="date")
        .join_asof(
